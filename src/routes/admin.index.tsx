...
function AdminDashboard() {
  ...
  const { data: stats } = useQuery({
    ...
    enabled: !!isAdmin,
    queryFn: async () => {
      const [{ count: products }, { count: producers }, { count: orders }, { count: subs }] =
        await Promise.all([
  ...
  const { data: b2bLeads } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("b2b_leads")
        .select(
          "id, company_name, contact_name, email, phone, estimated_quantity, purpose, status, created_at",
        )
        .order("created_at", { ascending: false })
        .limit(50);
      return data;
  ...
  const setProducerStatus = async (
    id: string,
    status: "active" | "rejected" | "suspended" | "pending_review",
  ) => {
    ...
    <div className="container mx-auto px-4 py-12 md:px-6">
      ...
      <section className="mt-12">
        <h2 className="font-display text-2xl text-primary">
          Orçamentos corporativos · Private Label
        </h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Leads recebidos pelo formulário Café com sua marca.
        </p>
                ...
                {(b2bLeads ?? []).map((l: any) => (
                  <tr key={l.id}>
                    ...
                    <td className="px-4 py-3 text-muted-foreground">
                      ...
                      <p className="line-clamp-1 text-xs">{l.purpose ?? ""}</p>
                    </td>
                    <td className="px-4 py-3">
                      <select
                        value={l.status}
                      ...
                      >
                        {["new", "contacted", "in_proposal", "won", "lost", "archived"].map(
                          (s) => (
            ...
            <tbody className="divide-y divide-border">
              {(pendingProducers ?? []).map((p: any) => (
                <tr key={p.id}>
                  ...
                  <td className="px-4 py-3 text-right">
                    <button
                      onClick={() => setProducerStatus(p.id, "active")}
                      className="mr-2 text-xs font-semibold text-[var(--farm)] hover:underline"
                    >
                      Aprovar
                    </button>
                    <button
                      onClick={() => setProducerStatus(p.id, "rejected")}
